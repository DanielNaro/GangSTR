...
bool BamInfoExtract::GetInsertSizeDistribution(std::map<std::string, SampleProfile> *profile,
    ...
    while (
        ...
        while (bamreader->GetNextAlignment(alignment)) {
            // Is this read worth looking at?
            if (alignment.IsSupplementary() || alignment.IsSecondary() || !alignment.IsProperPair()) {
                std::stringstream ss;
                ss << "Read " << alignment.Name()
                    << " discarded as not interesting."
                    <<" Supplementary:"<< alignment.IsSupplementary()
                    << "Secondary:"<< alignment.IsSecondary()
                    << "Proper pair:"<< alignment.IsProperPair();
                PrintMessageDieOnError(ss.str(), M_WARNING, false);
                continue;
            }
            // What sample did this read come from?
    ...
    for (std::set<std::string>::const_iterator it = samples.begin();
        ...
        if (tlen_vec.size() < min_reads_per_sample) {
            std::stringstream ss;
            ss << "Not enough reads for " << *it << " " << tlen_vec.size()
               << ". Please set insert size distribution manually.";
        ...
        if (tlen_vec_filt.size() < min_reads_per_sample) {
            ...
            ss << "Not enough reads for " << *it << " " << tlen_vec_filt.size()
               << " after filtering. Please set insert size distribution manually.";
            PrintMessageDieOnError(ss.str(), M_ERROR, false);
            return false;
        }
        // Set pdf/cdf
